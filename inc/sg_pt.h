 ...
 * If error in Unix returns negated errno. */
  int           scsi_pt_open_device( const char *device_name, int verbose );
 ...
 * In Win32 O_EXCL translated to equivalent. */
  int           scsi_pt_open_flags( const char *device_name, int flags,
      int verbose );
 ...
 * If error, returns negated errno (operating system) value. */
  int           check_pt_file_handle( int dev_fd, const char *device_name,
      int verbose );
 ...
 * descriptor. */
  struct sg_pt_base *construct_scsi_pt_obj_with_fd( int dev_fd, int verbose );
 ...
 * will return. dev_fd should be >= 0 for a valid file handle or -1 . */
  int           set_pt_file_handle( struct sg_pt_base *objp, int dev_fd,
      int verbose );
 ...
 * (out_true=true) or from (out_true=false) device (NVMe only) */
  void          set_pt_metadata_xfer( struct sg_pt_base *objp,
      uint8_t * mdxferp, uint32_t mdxfer_len, bool out_true );
...
/* The following "set_"s implementations may be dummies */
  ...
  void          set_scsi_pt_task_management( struct sg_pt_base *objp,
      int tmf_code );
 ...
 * be given to the obj constructor, or a prior set_pt_file_handle() call. */
  int           do_nvm_pt( struct sg_pt_base *objp, int submq,
      int timeout_secs, int verbose );
...
/* If not available return 0 (for success). */
  ...
  char         *get_scsi_pt_os_err_str( const struct sg_pt_base *objp,
 ...
 * The _actual_ values are related to resid (residual count from DMA) */
  void          get_pt_req_lengths( const struct sg_pt_base *objp,
      int *req_dinp, int *req_doutp );
  void          get_pt_actual_lengths( const struct sg_pt_base *objp,
      int *act_dinp, int *act_doutp );
